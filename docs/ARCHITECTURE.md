...
```
                                 ...
                                 v            v
                  +-------------------+    +-------------------------+
                  |    Cloudflare     |    |  Direct Port Forward    |
                  |   (DNS + CDN)     |    |  (Router -> Docker VM)  |
                  | *.jsamo.com       |    |  TCP:25565, UDP:19132   |
                  +-------------------+    +-------------------------+
                              |                        |
                              | Cloudflare Access      | Game traffic
                              | (Authentication)       | (Java + Bedrock)
                              v                        |
                  +-------------------+                |
                  | Cloudflare Tunnel |                |
                  |   (cloudflared)   |                |
...
|            v          v          v          v          v          v              |
|   +----------+ +----------+ +--------+ +----------+ +----------+ +----------+    |
|   |  Glance  | | Pi-hole  | |  N8N   | |  Uptime  | | MC Admin | | MC Backup|    |
|   |  :8080   | |:53,:8053 | | :5678  | |   Kuma   | |  :4326   | |          |    |
|   +----------+ +----------+ +--------+ |  :3001   | +----------+ +----------+    |
|                                        +----------+                              |
|                                                                                  |
|   +-------------------+                                                          |
|   |    Minecraft      |<------ Direct port forward (not via Traefik/Tunnel)      |
|   | :25565 (Java)     |                                                          |
|   | :19132 (Bedrock)  |                                                          |
|   +-------------------+                                                          |
|                                                                                   |
...
| 25565 | TCP      | Minecraft   | Java Edition (direct port forward) |
| 19132 | UDP      | Minecraft   | Bedrock Edition via GeyserMC (direct port forward) |
| 4326  | TCP      | MC Admin    | RCON web panel (via Traefik) |

Note: Minecraft game ports (25565, 19132) require direct router port forwarding to 192.168.0.100. Cloudflare Tunnel only supports HTTP/HTTPS traffic, so game traffic cannot be proxied through it. The MC Admin panel routes through Traefik and the tunnel like other web services.
